...
class gptSessionManage(object):
    ...
    def __init__(self,save_history):
        ...
        初始化
        '''
        self.messages = [{"role": "system", "content": "回答尽量不要超过400个字："}]
        self.sizeLim = save_history
        self.last_q_time = time.time()
        self.last_msg = ''

    def get_system_message(self):
        return next(msg['content'] for msg in self.messages if msg['role'] == 'system')

    def replace_system_content(self, new_content):
...
class gptMessageManage(object):
    ...
    def send_request(self,msgs):
        ...
        try:
            ...
            response = requests.post(self.url+'/chat/completions', headers=headers, json=json_data,timeout=14.2)
    ...
    def send_request_voice_stream(self,msgs):
        ...
        self.msgs_msgdata_dict[str(msgs.source)].replace_system_content('回答尽量不要超过300字：')
        ...
        timeout = self.configs['azure']['wait_time_out']
    ...
    def request_stream(self, headers,json_data,timeout):
        ...
        try:
            ...
            myrequest = requests.post(self.url+'/chat/completions', stream=True, headers=headers, json=json_data,timeout=timeout-0.8)
