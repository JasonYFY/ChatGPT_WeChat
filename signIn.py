...
#                 raise Exception('Invalid sendText. The list is empty.')
        try:
            ...
            # 创建页面对象，并启动或接管浏览器
            page = ChromiumPage(addr_driver_opts=co)
            page.get('https://lzkj-isv.isvjd.com/sign/signActivity2?activityId=79b1422c3d3d45a4a1e4696dda7212fb&venderId=1000013402')
            logger.info('打开页面了')
            time.sleep(3)
            # 第一次，需要登录才行 ，截图
            for i in range(3):
                swal = page.ele('#qr_area', timeout=2)
                if swal:

                    # 等待二维码出来
                    page.ele('#qrlogin_img')
                    logger.info('需要登录，准备截图发送')
                    image_data = page.get_screenshot(as_base64='jpg')
                    sendEmail.send('请扫码登录：', image_data)
                    logger.info('睡眠30s')
...
#                     time.sleep(2)
            ...
            logger.error('singInOfYiLi报错：%s', e)
            image_data = page.get_screenshot(as_base64='jpg')
            sendEmail.send(f'singInOfYiLi报错：{e}', image_data)
        finally:
            try:
                page.quit()
            except Exception as e:
                logger.debug('singInOfYiLi退出浏览器正常报错，不用理：%s', e)
